...
class HttpsRedirectFilter @Inject()(implicit val mat: Materializer, ec: ExecutionContext, env: play.api.Environment, config: play.api.Configuration) extends Filter {
  ...
  def isProperMode = !config.getBoolean("httpsRedirect.productionOnly").getOrElse(true) || env.mode == Mode.Prod
  ...
  def apply(nextFilter: RequestHeader => Future[Result])
    ...
    if (!isRequestSecure(requestHeader) && isProperMode && !isExcluded(requestHeader.uri)) {
